...
namespace gird {
        ...
        const ColumnDef* GridController::FindCol1(const std::string& id) const
        {
        ...
        static int cmp_values_string(const Value& a, const Value& b)
        {
            const auto* sa = std::get_if<std::string>(&a);
            const auto* sb = std::get_if<std::string>(&b);
            const std::string va = sa ? *sa : std::string{};
            const std::string vb = sb ? *sb : std::string{};
            if (va < vb) return -1;
            if (va > vb) return  1;
            return 0;
        }
        ...
        void GridController::RebuildIndices()
        {
            if (!doc || !vm || !doc->source) return;
            ...
            for (auto it = vm->groupByColumnIds.rbegin(); it != vm->groupByColumnIds.rend(); ++it) {
                ...
                for (auto& k : effective)
                    if (k.column_id == *it) { already = true; break; }
                if (!already)
            ...
            if (!effective.empty())
            {
                std::stable_sort(vm->indices.begin(), vm->indices.end(),
                    [&](int ra, int rb) {
                        const auto& rowA = doc->source->RowAt(ra);
                        const auto& rowB = doc->source->RowAt(rb);

                        for (const auto& key : effective) {
                            const ColumnDef* col = FindCol1(key.column_id);
                            if (!col || !col->getValue) continue;
        ...
        void GridController::RebuildGroups()
        {
            ...
            BuildGroupLevel(0, 0, (int)vm->indices.size(), 0);
...
static const char* AggTypeName(gird::AggType t) {
    switch (t) {
        case gird::AggType::Count: return "Count";
        case gird::AggType::Min:   return "Min";
        case gird::AggType::Max:   return "Max";
        case gird::AggType::Sum:   return "Sum";
        case gird::AggType::Avg:   return "Avg";
        case gird::AggType::Custom:return "Custom";
        default: return "?";
    }
}
        ...
        void GridController::RebuildViewColumns()
        {
            ...
            for (const auto& a : vm->active_aggs) {
                ...
                const ColumnDef* base = FindCol1(a.column_id);
                const std::string base_label = base ? base->label : a.column_id;
                vc.label = std::string(AggTypeName(a.type)) + "(" + base_label + ")";
...
{
    if (level >= (int)vm->groupByColumnIds.size()) {
    ...
    while (i < end)
    {
        ...
        // Emit group header for this run
        {
            ...
            const int node_idx = (int)vm->groupNodes.size();
        ...
        int GridController::FindColumn(const GridDocument& doc, const std::string& id) const
        {
            for (int c = 0; c < (int)doc.columns.size(); ++c)
                if (doc.columns[c].id == id) return c;
            return -1;
        }

        // Extract group key as string (for comparison + display)
        std::string GridController::GetGroupKey(const GridDocument& doc, int col_idx, const SimpleRow& row) const
        {
            const auto& col = doc.columns[col_idx];
            const Value v = col.getValue(row);
            return col.format(v); // reuse your existing format function
        }
       ...
       std::vector<std::string> GridController::ComputeSummaries(int begin, int end) const
{
    ...
    {
        if (doc_col_index < 0 || doc_col_index >= (int)doc->columns.size()) return {};
        ...
        if (col.type == ValueType::Int64)
        {
            ...
            switch (type) {
                case AggType::Min: return format_i64(mn);
                case AggType::Max: return format_i64(mx);
                case AggType::Sum: return format_i64(sum);
                case AggType::Avg: return format_f64((double)sum / (double)count);
                default: return {};
            }
        }
        else if (col.type == ValueType::Double)
        {
            ...
            switch (type) {
                case AggType::Min: return format_f64(mn);
                case AggType::Max: return format_f64(mx);
                case AggType::Sum: return format_f64(sum);
                case AggType::Avg: return format_f64(sum / (double)count);
                default: return {};
            }
        }
    ...
    for (int vc = 0; vc < (int)vm->viewColumns.size(); ++vc)
    {
        const auto& vcol = vm->viewColumns[vc];
        if (vcol.kind != ViewColumn::Kind::Agg) continue;

        // Find source doc column by id
        int doc_col_index = -1;
        for (int c = 0; c < (int)doc->columns.size(); ++c) {
            if (doc->columns[c].id == vcol.agg.column_id) { doc_col_index = c; break; }
        }
